...
const RateCard = () => {
  ...
  const downloadCSV = (allCouriers, allZones, existingData) => {
    ...
    const rows = allCouriers.map((courier) => {
      ...
      const zoneValues = allZones.flatMap((zone) => {
        ...
        return [
          zoneRates.forward != null ? `â‚¹${zoneRates.forward}` : '',
          zoneRates.rto != null ? `â‚¹${zoneRates.rto}` : '',
        ]
      })
      ...
      return [
        ...
        courier,
        ...zoneValues,
        row.cod_charges != null ? `â‚¹${row.cod_charges}` : '',
        row.cod_percent != null ? `${row.cod_percent}%` : '',
        row.other_charges != null ? `â‚¹${row.other_charges}` : '',
      ]
    })
  ...
  const zoneColumns = useMemo(() => {
    ...
    return zones.map((zone) => ({
      ...
      renderer: (_, row) => {
        const zoneRates = row.rates?.[zone.name] || {}
        return `${zoneRates.forward != null ? `â‚¹${zoneRates.forward}` : 'NA'} | ${
          zoneRates.rto != null ? `â‚¹${zoneRates.rto}` : 'NA'
        }`
      },
    }))
  }, [zones])
    ...
    () => [
      {
        ...
        width: '200px',
        renderer: (_, row) =>
          `${row.cod_charges != null ? `â‚¹${row.cod_charges}` : 'NA'} | ${
