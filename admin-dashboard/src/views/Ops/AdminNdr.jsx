    ...
    <Flex direction="column" pt={{ base: '12px', md: '4px' }} gap={4}>
      ...
      <GenericTable
        ...
        renderers={{
          ...
          last_event_time: (v) => (
            <Text fontSize="xs">{v ? new Date(v).toLocaleString() : 'â€”'}</Text>
          ),
          remarks: (v, row) => (
            <Stack spacing={0}>
              <HStack spacing={2}>
              ...
              <Tooltip
                ...
                openDelay={250}
              >
                <Text noOfLines={1}>{row?.reason || 'â€”'}</Text>
              </Tooltip>
              <Tooltip
                label={row?.remarks || 'â€”'}
                hasArrow
          ...
          <Stack spacing={3}>
            {events.map((e, idx) => (
              <Stack key={idx} spacing={0} borderLeft="2px solid #E2E8F0" pl={3}>
                ...
                <Text fontWeight="600">{e?.status || 'â€”'}</Text>
                {e?.remarks && (
