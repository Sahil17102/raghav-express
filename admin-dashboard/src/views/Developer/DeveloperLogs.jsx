  ...
  const renderers = {
    created_at: (_value, row) => (
      <Box maxW="220px">
        ...
        <Text fontSize="xs" color="gray.500">
          Repeat count: {row.occurrence_count} â€¢ Orders: {row.affected_order_count || 0}
        </Text>
      </Box>
    ),
            ...
            {alerts.map((alert) => (
              ...
              >
                <Box>
                  ...
                  <Text fontSize="sm" color={mutedColor}>
                    {alert.root_cause_label} â€¢ {alert.why_this_matters}
                  </Text>
                  <Text fontSize="xs" color="gray.500">
                    {alert.merchant_name || 'Unknown merchant'} â€¢ Last seen {formatDateTime(alert.last_seen_at)}
                  </Text>
                </Box>
                  ...
                  <Box key={order.id} p={3} borderWidth="1px" borderRadius="lg">
                    <Flex justify="space-between" gap={3} wrap="wrap">
                      <Box>
                        ...
                        <Text fontSize="sm" color={mutedColor}>
                          Status: {order.order_status || 'unknown'} â€¢ Repeat count: {order.occurrence_count}
                        </Text>
                        <Text fontSize="sm" color={mutedColor}>
              ...
              <VStack align="stretch" spacing={2}>
                {issue.related_pending_webhooks.map((row) => (
                  <Box key={row.id} p={3} borderWidth="1px" borderRadius="lg">
                    <Text fontWeight="700">
                      {row.awb_number || 'Unknown AWB'} â€¢ {row.status || 'pending'}
                    </Text>
                    <Text fontSize="sm" color={mutedColor}>
                      Record ID: {row.id}
                    </Text>
            ...
            <Card p={4} bg={cardBg}>
              ...
              <VStack align="stretch" spacing={2}>
                {issue.related_webhook_deliveries.map((row) => (
                  <Box key={row.id} p={3} borderWidth="1px" borderRadius="lg">
                    <Text fontWeight="700">
                      {row.event_type || 'Event'} â€¢ Attempt {row.attempt_count}/{row.max_attempts}
                    </Text>
                    <Text fontSize="sm" color={mutedColor}>
                      Delivery ID: {row.id} â€¢ Event ID: {row.event_id || 'Unknown'}
                    </Text>
                    <Text fontSize="sm" color={mutedColor}>
                      HTTP status: {row.http_status || 'No response'} â€¢ Failed at {formatDateTime(row.failed_at)}
                    </Text>
                  </Box>
                ))}
              </VStack>
                ...
                {issue.audit_trail.map((item) => (
                  <Box key={item.id} p={3} borderWidth="1px" borderRadius="lg">
                    ...
                    <Text fontSize="sm" color={mutedColor}>
                      ...
                      {item.admin_email ? ` (${item.admin_email})` : ''} â€¢ {formatDateTime(item.created_at)}
                    </Text>
                    {item.metadata ? (
