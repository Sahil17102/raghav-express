...
export default function AdminCodRemittancePage() {
  ...
  return (
    <Flex direction="column" pt={{ base: '12px', md: '4px' }}>
      {/* Stats Cards */}
        ...
        <Card>
          <Stat>
            ...
            <StatNumber fontSize="2xl" fontWeight="bold" color="orange.500">
              â‚¹{stats?.data?.totalPending?.amount?.toLocaleString('en-IN') || 0}
            </StatNumber>
        ...
        <Card>
          <Stat>
            ...
            <StatNumber fontSize="2xl" fontWeight="bold" color="green.500">
              â‚¹{stats?.data?.todayCredited?.amount?.toLocaleString('en-IN') || 0}
            </StatNumber>
            <StatHelpText fontSize="xs" color="gray.400">
        ...
        <Card>
          <Stat>
            ...
            <StatNumber fontSize="2xl" fontWeight="bold" color="blue.500">
              â‚¹{stats?.data?.totalCredited?.amount?.toLocaleString('en-IN') || 0}
            </StatNumber>
            <StatHelpText fontSize="xs" color="gray.400">
      ...
      <GenericTable
        ...
        renderers={{
          ...
          codAmount: (value) => <Text fontWeight="600">â‚¹{value?.toLocaleString('en-IN')}</Text>,
          deductions: (value) => <Text color="red.500">â‚¹{value?.toLocaleString('en-IN')}</Text>,
                        ...
                        {sellerPendingRemittances.map((item) => (
                          <Tr key={item.id}>
                            ...
                            <Td fontSize="xs" fontWeight="600">
                              â‚¹{toAmount(item.remittableAmount).toLocaleString('en-IN')}
                            </Td>
                            <Td minW="160px">
                              <Input
                                size="xs"
          ...
          <ModalBody>
            <VStack spacing={4} align="stretch">
              ...
              <Box bg="blue.50" p={3} borderRadius="md" fontSize="xs">
                ...
                <Text>2. Navigate to Reports â†’ COD Settlement</Text>
              ...
              <VStack spacing={4} align="stretch">
                ...
                <SimpleGrid columns={{ base: 2, md: 4 }} spacing={3}>
                  ...
                  >
                    ...
                    <Text fontSize="xs" color="gray.500">
                      â‚¹{csvPreviewData.summary.totalMatchedAmount?.toLocaleString('en-IN')}
                    </Text>
                  </Box>
                ...
                <Tabs index={reviewTabIndex} onChange={setReviewTabIndex}>
                  <TabList>
                    <Tab>Matched ({csvPreviewData.results.matched.length}) âœ…</Tab>
                    <Tab>Discrepancies ({csvPreviewData.results.discrepancies.length}) âš ï¸</Tab>
                    <Tab>Not Found ({csvPreviewData.results.notFound.length}) âŒ</Tab>
                              ...
                              <Tbody>
                                {csvPreviewData.results.matched.map((item) => (
                                  <Tr key={item.remittanceId}>
                                    ...
                                    <Td fontSize="xs">
                                      â‚¹{item.ourAmount?.toLocaleString('en-IN')}
                                    </Td>
                                    <Td fontSize="xs" color="green.600">
                                      â‚¹{Math.abs(getEditedAmount(item) - toAmount(item.ourAmount)).toFixed(2)}
                                    </Td>
                                    <Td>
                                  ...
                                  <Tr key={item.remittanceId} bg="orange.50">
                                    ...
                                    <Td fontSize="xs" fontWeight="600" color="red.600">
                                      {item.difference !== null && item.difference !== undefined
                                        ? `${getEditedAmount(item) - toAmount(item.ourAmount) > 0 ? '+' : ''}â‚¹${(
                            ...
                            <Tbody>
                              {csvPreviewData.results.notFound.map((item, index) => (
                                <Tr key={index} bg="red.50">
                                  ...
                                  <Td fontSize="xs">{item.orderNumber}</Td>
                                  <Td fontSize="xs">
                                    â‚¹{item.courierAmount?.toLocaleString('en-IN')}
                                  </Td>
                                </Tr>
                              ))}
                            </Tbody>
                            ...
                            <Tbody>
                              {csvPreviewData.results.alreadyCredited.map((item) => (
                                <Tr key={item.remittanceId} bg="gray.50">
                                  ...
                                  <Td fontSize="xs" color="green.600">
                                    â‚¹{item.creditedAmount?.toLocaleString('en-IN')}
                                  </Td>
                                </Tr>
                              ))}
                            </Tbody>
