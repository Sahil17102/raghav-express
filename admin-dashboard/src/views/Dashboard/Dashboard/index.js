...
import { BRAND, brandGradient, brandBackgroundSize } from '../../../constants/brand'
...
function SectionCard({ title, subtitle, children, panelBg, borderColor, headerAction }) {
  ...
  const accent = title.includes('Performance') || title.includes('Delivery')
    ? '#0F8A72'
    : title.includes('Revenue') || title.includes('Financial')
    ? '#ED1C24'
    : title.includes('Seller') || title.includes('User')
    ? '#6D4DC2'
    : '#0B5BB5'
  ...
  return (
    <Card
      ...
      borderRadius="10px"
      h="full"
      boxShadow="0 10px 28px rgba(6, 26, 51, 0.065)"
    >
      <CardHeader p={5} pb={children ? 2 : 5}>
        <Flex align="flex-start" justify="space-between" gap={3}>
          <Box>
            <HStack spacing={2}>
              <Box w="8px" h="8px" borderRadius="full" bg={accent} boxShadow={`0 0 0 4px ${accent}18`} />
...
function MetricInfoTile({ label, value, helper, tone = 'teal', panelBg, borderColor }) {
  ...
  const textSecondary = useColorModeValue('gray.600', 'gray.400')
  const accentMap = {
    teal: { soft: '#E8F2FB', strong: '#062A5B' },
    orange: { soft: '#FDE7EA', strong: '#ED1C24' },
    blue: { soft: '#E7F1FF', strong: '#0B5BB5' },
    green: { soft: '#E5F7F1', strong: '#0F8A72' },
    red: { soft: '#FDE8EA', strong: '#C81825' },
  }
  const selectedTone = accentMap[tone] || accentMap.teal
  ...
  return (
    <Box
      ...
      borderRadius="10px"
      borderWidth="1px"
      borderColor={`${selectedTone.strong}28`}
      bg={`linear-gradient(145deg, ${panelBg} 0%, ${selectedTone.soft} 100%)`}
      boxShadow="0 8px 22px rgba(6, 26, 51, 0.055)"
    >
      <Box
        w="34px"
        h="34px"
        borderRadius="10px"
        bg={selectedTone.strong}
        boxShadow={`0 7px 16px ${selectedTone.strong}38`}
        mb={3}
      />
      <Text fontSize="xs" textTransform="uppercase" letterSpacing="0.08em" color={textSecondary} fontWeight="700">
        {label}
      </Text>
      <Text mt={1.5} fontSize="2xl" fontWeight="800" color={selectedTone.strong}>
        {value}
      </Text>
...
export default function Dashboard() {
  ...
  const { data: statsData, isLoading, error, refetch, isRefetching, dataUpdatedAt } = useDashboardStats()
  ...
  return (
    <Box minH="100vh" pb={8} bg={pageBg} backgroundSize={brandBackgroundSize} backgroundAttachment="fixed">
          ...
          <PageHeader
            ...
            actions={
              <Stack direction={{ base: 'column', md: 'row' }} spacing={3} w="100%" justify={{ xl: 'flex-end' }}>
                ...
                >
                  <Box w="8px" h="8px" borderRadius="full" bg="green.500" boxShadow="0 0 0 4px rgba(22,163,74,0.12)" />
                  <Box>
                    <Text fontSize="10px" fontWeight="800" color="green.700" letterSpacing="0.08em">LIVE ANALYTICS</Text>
                    <Text fontSize="10px" color="green.700">
                      Updated {dataUpdatedAt ? new Date(dataUpdatedAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : 'just now'}
                    </Text>
                  </Box>
                </HStack>
                <Button
            ...
            <VStack spacing={4} align="stretch">
              {performanceMetrics.map((metric) => (
                <Box
                  ...
                  borderLeftColor={`${metric.colorScheme}.500`}
                >
              ...
              <Box p={4} borderRadius="10px" borderWidth="1px" borderColor="blue.200" borderLeftWidth="4px" borderLeftColor="blue.500" bg="blue.50">
